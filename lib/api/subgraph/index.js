  ...
  while (hasMore) {
    const response = await graphql({ ...params, api_type: 'analytic', query: `
      {
        hourlyMetrics(orderBy: hourStartTimestamp, orderDirection: desc, skip: ${skip}, first: ${size}${where ? `, where: ${where}` : ''}) {
          ...
          assetId
          volume
          ${old_schema_chains.includes(params?.chain_id) ? '' : 'sendingTxCount'}
          ${old_schema_chains.includes(params?.chain_id) ? '' : 'receivingTxCount'}
          ${old_schema_chains.includes(params?.chain_id) ? '' : 'cancelTxCount'}
          ${old_schema_chains.includes(params?.chain_id) ? '' : 'volumeIn'}
        }
      }
    ` })
    ...
    data = _.uniqBy(_.concat(data || [], response?.data?.hourlyMetrics?.map(hourlyMetric => {
      return {
        ...
        sendingTxCount: Number(hourlyMetric.sendingTxCount) || 0,
        receivingTxCount: Number(hourlyMetric.receivingTxCount) || 0,
        cancelTxCount: Number(hourlyMetric.cancelTxCount) || 0,
        volumeIn: Number(hourlyMetric.volumeIn) || 0,
      }
    })), 'id')
  ...
  while (hasMore) {
    const response = await graphql({ ...params, api_type: 'analytic', query: `
      {
        dayMetrics(orderBy: dayStartTimestamp, orderDirection: desc, skip: ${skip}, first: ${size}${where ? `, where: ${where}` : ''}) {
          ...
          ${old_schema_chains.includes(params?.chain_id) ? '' : 'cancelTxCount'}
          ${old_schema_chains.includes(params?.chain_id) ? '' : 'volumeIn'}
          ${old_schema_chains.includes(params?.chain_id) ? '' : 'relayerFee'}
        }
      }
    ` })
    ...
    data = _.uniqBy(_.concat(data || [], response?.data?.dayMetrics?.map(dayMetric => {
      return {
        ...
        relayerFee: Number(dayMetric.relayerFee) || 0,
      }
    })), 'id')
    ...
    const response = await graphql({ ...params, api_type: 'analytic', query: `
      {
        assetBalances(orderBy: amount, orderDirection: desc) {
          ...
          assetId
          ${old_schema_chains.includes(params?.chain_id) ? '' : 'locked'}
          ${old_schema_chains.includes(params?.chain_id) ? '' : 'lockedIn'}
          ${old_schema_chains.includes(params?.chain_id) ? '' : 'supplied'}
          ${old_schema_chains.includes(params?.chain_id) ? '' : 'removed'}
          ${old_schema_chains.includes(params?.chain_id) ? '' : 'volume'}
          ${old_schema_chains.includes(params?.chain_id) ? '' : 'volumeIn'}
          ${old_schema_chains.includes(params?.chain_id) ? '' : 'receivingFulfillTxCount'}
        }
      }
    ` })
