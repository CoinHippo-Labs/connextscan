...
export const dayMetrics = async params => {
  const path = `/day_metrics${process.env.NEXT_PUBLIC_NETWORK ? `_${process.env.NEXT_PUBLIC_NETWORK}` : ''}/_search`
      ...
      data: Object.fromEntries(response.aggregations.chains.buckets.map(chain => {
        const records = chain?.day_metrics?.buckets?.map(day => {
          return {
            ...
            receivingTxCount: day?.receiving_txs?.value || day?.txs?.value,
            cancelTxCount: day?.cancel_txs?.value,
            _normalize_volumeIn: day?.volume_ins?.value,
            _normalize_relayerFee: day?.relayer_fees?.value,
