...
export const numberFormat = (number, format, is_exact) => {
  let _number = numberOptimizeDecimal(numeral(number).format(format.includes('.000') && Math.abs(Number(number)) >= 1.01 ? `${format.substring(0, format.indexOf('.') + (is_exact ? 4 : 3))}` : format === '0,0' && Number(number) < 1 ? '0,0.00' : format))
