	...
	return (
      ...
      {navigations.map((item, i) => (
        <Link key={i} href={`${chain_id ? `/${chain_id}` : ''}${item.path}`}>
          <a className={`bg-transparent hover:bg-gray-100 dark:hover:bg-gray-800 rounded flex items-center uppercase text-xs xl:text-sm space-x-1 p-2 ${pathname === `${chain_id ? `/${chain_id}` : ''}${item.path}` ? 'text-gray-900 hover:text-gray-800 dark:text-gray-100 dark:hover:text-gray-200 font-bold' : 'text-gray-600 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300 font-medium'}`}>
        		{item.icon}
